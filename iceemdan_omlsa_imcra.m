...
[clean,fs, nbits]= wavread('SA1FCMR0.wav');          % parole propre

[signal,fs0,nbits]= wavread('SA1FCMR0_BBL_sn0.wav');  
...
%% 
% Parameters of ICEEMDAN : Improved Complete EEMD with Adaptive Noise
% *********************************************************** 
MaxIter1 = 10; % defines the maximum number of sifting iterations
Nstd = 0.2;
NR = 100;
SNRFlag=1; 
[modes its]=ceemdan2(signal,Nstd,NR,MaxIter1,SNRFlag); 
[a b]=size(modes)

%%
% Parameters of EMD : Empirical Mode Decomposition 
% ***********************************************************
% a = 10;
% MaxIter = 2000; % defines the maximum number of sifting iterations
% modes = emd(signal,'MAXMODES',a,'MAXITERATIONS',MaxIter)
...
wavwrite(IMF,16000,16,'IMFnoisy.wav');
...
% Read input data
[y,Fs]=audioread('IMFnoisy.wav');  % read size of input data, Fs and NBITS
N=length(y);
N = N(1);
...
for l=1:Nframes
    ...
    if l==1
        foutID = fopen(['IMFenhanced' '.wav'],'w');      % open output file and write first output samples
...
%%         
[yt,Srate,nbits]=wavread('IMFenhanced.wav');
...
for i=1:lag:len-window
    fram = IMFenh1(1:a,i:i+window-1);
    counterIMF = 3;
    H = 0;
    while (H < Hth && counterIMF < a)
        counterIMF = counterIMF + 1;
        estim = LDestimate(fram(counterIMF,:),1,1,8,0,0,0);
        H = abs(estim-1)/2;
    end;
    windo = sum(fram(1:counterIMF,:));
    yy(i:i+window-1) = yy(i:i+window-1) + windo.*ww';
 counter = counter + 1;
end;
...
i = i+lag;
fram = IMFenh1(1:a,i:len);
windo = sum(fram(1: counterIMF,:));
ww(lag+1:window) = ones(lag,1);
yy(i:len) = yy(i:len) + windo.*ww(1:len-i+1)';
 
wavwrite(yy,fs,nbits,'SA1FCMR0enhanced.wav');


[cLLR cSNRseg cWSS cPESQ]= mcomposite('SA1FCMR0.wav','SA1FCMR0enhanced.wav')
